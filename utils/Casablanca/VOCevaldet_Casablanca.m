function [rec,prec,ap] = VOCevaldet_Casablanca(VOCopts,id,cls,draw)
...
if exist(cp,'file')
    ...
    for i=1:length(gtids)
        ...
        % read annotation
        rec_t = VOCreadrecxml(sprintf(VOCopts.annopath,gtids{i}));
        if ~isfield(rec_t, 'objects')
            rec_t.objects(1).class = 'dummy';
            rec_t.objects(1).bbox = [0 0 0 0];
            rec_t.objects(1).difficult = 1;            
        end
        recs(i) = rec_t;
    end
    save(cp,'gtids','recs');
...
% hash image ids
hash=VOChash_init_Casablanca(gtids);
...
for d=1:nd
    ...
    i=VOChash_lookup_Casablanca(hash,ids{d});
    if isempty(i)
