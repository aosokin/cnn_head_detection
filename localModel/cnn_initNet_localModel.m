function net = cnn_initNet_localModel( networkFile, numClasses, meanImage, numNodesExtraLayers, addDropout )
%cnn_initNet_localModel initializes CNN from a pretrained network in MatConvNet format
%
% net = cnn_initNet_localModel( networkFile, numClasses);
% net = cnn_initNet_localModel( networkFile, numClasses, meanImage );
% net = cnn_initNet_localModel( networkFile, numClasses, meanImage, numNodesExtraLayers );
% net = cnn_initNet_localModel( networkFile, numClasses, meanImage, numNodesExtraLayers, addDropout );
%
% Input: 
...
net.layers{end+1} = struct('type', 'conv', ...
    'weights', {{ 0.01/scal * randn(1, 1, numNodesExtraLayers(end), numClasses,'single'),... % filters
      init_bias*ones(1,numClasses,'single') }}, ... % biases
    'stride', 1, ...
    'pad', 0, ...
    'learningRate', [1, 2], ...
    'weightDecay', [1, 0], ...
